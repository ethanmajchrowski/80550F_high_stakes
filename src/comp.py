...
class AutonomousHandler:
    ...
    def run(self):
        ...
        self.fwd_speed = 7.5
        self.path(paths[1], backwards=True, finish_margin=200, look_ahead_dist=200)

        doinker_pneu.set(False)
        drivetrain.turn_for(TurnType.LEFT, 90, RotationUnits.DEG, 100, VelocityUnits.PERCENT)
        self.fwd_speed = 8
        self.path(paths[2], backwards=True, heading_authority=2)
        mogo_pneu.set(True)
        ...
        self.path(paths[3], backwards=False, finish_margin=170)
        sleep(1000, MSEC)
        motors["misc"]["intake_chain"].stop()
        ...
        self.path(paths[4], backwards=True, heading_authority=2, timeout=2000)
        mogo_pneu.set(True)
        ...
        self.path(paths[5], look_ahead_dist=350, timeout = 2000, heading_authority=3)

        self.fwd_speed = 4
        self.path(paths[6], backwards=True)
        ...
        drivetrain.turn_for(TurnType.LEFT, 180, RotationUnits.DEG, 100, VelocityUnits.PERCENT)
        self.fwd_speed = 8
        self.path(paths[7], backwards=False, finish_margin=150, timeout=1300)
