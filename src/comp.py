...
class Autonomous():
    ...
    def test(self) -> None:
        ...
        # grab mogo path
        self.fwd_speed = 6
        self.path(paths[1], backwards=True, heading_authority=1, slowdown_distance=0, speed_ramp_time=300)
        ...
        # drop mogo in corner
        self.path(paths[3], backwards=True, timeout=1300, heading_authority=2)
        ...
        if d1 < 900:
            ...
            log("Recalibrate x pos from {} to {}. Diff: {}".format(self.robot.pos[0], x, diff))
            self.robot.pos[0] = x
        else:
        ...
        self.fwd_speed = 7
        self.path(paths[4], slowdown_distance=1000)
        # grab top left mogo
        self.fwd_speed = 6
        self.path(paths[5], backwards=True, speed_ramp_time=400)
        pneumatic.mogo.set(True)
        sleep(300, TimeUnits.MSEC)
        # Intake and grab top left rings
        motor.intakeChain.spin(DirectionType.FORWARD, 100, VelocityUnits.PERCENT)
        self.fwd_speed = 6
        self.path(paths[6], speed_ramp_time=400, checkpoints=[15, 30, 40, 60, 70], slowdown_distance=700, min_slow_voltage=5)
        # drop mogo in corner
        self.path(paths[7], backwards=True, timeout=850)
        pneumatic.mogo.set(False)
        ...
        # top blue goal corner shove
        self.path(paths[8], speed_ramp_time=1000, min_start_voltage=2.5, timeout=7000, slowdown_distance=1000, look_ahead_dist=400,)
                  #events=[["pres intake", (1440.0, 710.0), pneumatic.intake.set, (True,)]])
        motor.intakeChain.stop(BrakeType.COAST)
        ...
        log("Recalibrated to {}".format(self.robot.pos))

        AutonomousFlags.intake_auto_halt = False
        AutonomousFlags.intake_flex_auto_halt = False
        self.fwd_speed = 6
        # back out of corner
        self.path(paths[9], backwards=True, speed_ramp_time=300, heading_authority=1.8)
        pneumatic.mogo.set(True)
        sleep(300, TimeUnits.MSEC)

        motor.intakeChain.spin(DirectionType.FORWARD, 100, VelocityUnits.PERCENT)
        motor.intakeFlex.spin(DirectionType.FORWARD, 100, VelocityUnits.PERCENT)
        self.path(paths[10], speed_ramp_time=400, checkpoints=[30, 50], event_look_ahead_dist=150,
                  events=[
        ...
        self.path(paths[12], speed_ramp_time=400, slowdown_distance=800)
        motor.intakeChain.stop()
        self.fwd_speed = 7
        self.path(paths[13], backwards=True, speed_ramp_time=400, timeout=1500)
        ...
        x += 140
        self.robot.pos[0] = 1785 - x
        log("Recalibrated x to {}".format(self.robot.pos[0]))
        ...
        self.fwd_speed = 8
        self.path(paths[14], speed_ramp_time=600, slowdown_distance=800, timeout=3500, heading_authority=1.5)
        ...
        self.fwd_speed = 8
        self.path(paths[15], backwards=True, speed_ramp_time=500, slowdown_distance=600)
        ...
        self.fwd_speed = 11
        # self.robot.pos = [650.0, 758.0]
        self.path(paths[16], timeout=2200, speed_ramp_time=400)
...
class ControllerFunctions():
    ...
    def elevation_bar():
        ...
        motor.intakeChain.spin(DirectionType.REVERSE, 80, VelocityUnits.PERCENT)
        # print(motor.ladyBrown.command())
